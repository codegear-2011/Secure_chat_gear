...
1. **Download the installation package**:
   ```bash
   git clone https://github.com/developerrahulofficial/Secure-chat-client.git
...
3. **Verify installation**:
   ```bash
   sudo systemctl status secure-chat-client
   ```
...
2. **Create application directory**:
   ```bash
   sudo mkdir -p /opt/secure-chat-client
   sudo cp server.py /opt/secure-chat-client/
   sudo chmod +x /opt/secure-chat-client/server.py
   ```

3. **Create systemd service** (optional but recommended):
   ```bash
   sudo cp secure-chat-client.service /etc/systemd/system/
   sudo systemctl daemon-reload
   sudo systemctl enable secure-chat-client
   sudo systemctl start secure-chat-client
   ```
...
Before using Secure Chat, you need to generate a PGP key pair. This section provides step-by-step instructions for creating your cryptographic keys using GnuPG (GPG).
...
Both files can be directly uploaded to the Secure Chat application.
...
1. **Access the application** by opening `index.html` in your web browser
2. **Connect to server** by entering the WebSocket URL (e.g., `ws://your-server:8765` , or my live server  : `https://secure-chat-1-vvo6.onrender.com` )
